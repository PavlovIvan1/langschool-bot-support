...
async def command_start_handler(message: Message, state: FSMContext) -> None:
    ...
    if len(user_data) == 0:
        ...
        await message.answer_document(FSInputFile('files/Инструкция_по чат-боту.pdf'), caption='''Добро пожаловать в бот проверки домашних заданий на курсе "Заработок на Reels"
Напиши свою электронную почту, которую ты указывал(ла) при покупке обучения, чтобы мы посмотрели тебя в списке учеников''')
        
        return
    
    await message.answer('Добро пожаловать в бот проверки домашних заданий на курсе "Заработок на Reels"', reply_markup=keyboard.main_keyboard())

@start_router.message(F.text == '/skip_state')
async def command_start_handler(message: Message, state: FSMContext) -> None:
    print(f"Состояние до clear(): {await state.get_state()}")
    await state.clear()
    print(f"Состояние после clear(): {await state.get_state()}")  # Должно быть None
    await message.answer("Стейт сброшен")

@start_router.message(F.text.startswith('/get_msg'))
async def command_start_handler(message: Message) -> None:
    if message.from_user.id != 935173049:
        return
    ...
    _, tg_id, msg_id = message.text.split()
    ...
    for i in range(1, 501):
        try:
            await message.bot.copy_message(message.from_user.id, int(tg_id), int(msg_id)+i)
            break
        except:
            print(f"Сообщение не найдено: {i}")

        await asyncio.sleep(1)

@start_router.message(F.text.startswith('/почта'))
async def command_start_handler(message: Message) -> None:
    if message.from_user.id not in [935173049, 580485261]:
        return
    ...
    is_access = db.is_email_in_users_access(email.lower())

    if is_access:
        await message.answer(f'Почта пользователя <code>{email}</code> <b>есть</b> в списке учеников', parse_mode='html')
    else:
        await message.answer(f'Почты пользователя <code>{email}</code> <b>нет</b> в списке учеников', parse_mode='html')


@start_router.message(F.text.startswith('/check_status'))
async def command_start_handler(message: Message) -> None:
    if message.from_user.id != 935173049:
        return
    
    homeworks_ids = message.text.split()[1:]

    for homework_id in homeworks_ids:
        if not homework_id.isdigit():
            continue
        
        homework_data = db.get_homework(homework_id)

        if homework_data is None:
            continue

        if homework_data['comment'].startswith('+') and len(homework_data['comment'].splitlines()) == 1: # Трекер принял задание
            user_name = ''

            try:
                user_data = await message.bot.get_chat(int(homework_data['user_data'].split()[-1]))
                user_name = user_data.first_name
            except:
                pass

            db.edit_homework(homework_data["homework_id"], status='✅', comment=f'{user_name}, ты классно выполнила домашние задание. Корректировок нет, я принимаю его.', check_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        elif homework_data['comment'].splitlines()[0].startswith('+'): # Трекер принял задание
            message_text = homework_data['comment'][1:].strip()
            db.edit_homework(homework_data["homework_id"], status='✅', comment=message_text, check_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        elif homework_data['comment'] == '1': # Трекер отправил на проверку
            db.edit_homework(homework_data["homework_id"], status='На проверке', check_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        else:
            db.edit_homework(homework_data["homework_id"], status='❌', comment=homework_data['comment'], check_time=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))

        try:
            if homework_data['comment'] == '+' or homework_data['comment'].splitlines()[0] == '+':
                await send_congratulation_message(message, int(homework_data['lesson_id']), int(homework_data['user_data'].split()[-1]))

            await message.bot.send_message(
                int(homework_data['user_data'].split()[-1]),
                "Твой персональный трекер проверил твое задание🔥 Нажми на кнопку, чтобы посмотреть обратную связь👇🏻",
                reply_markup=keyboard.open_lesson_keyboard(homework_data['lesson_id']) if homework_data['comment'] == '+' or homework_data['comment'].splitlines()[0] == '+' else keyboard.get_last_solution_keyboard(int(homework_data['lesson_id']))
            )
        except:
            pass
...
@start_router.message(F.text.startswith('/send_msg '))
async def command_start_handler(message: Message) -> None:
    if message.from_user.id != 935173049:
        return
    ...
    _, tg_id, msg_id, module_id, lesson_id = message.text.split()
    ...
    is_do_homework = db.get_homework_by_lesson_id(tg_id, lesson_id)
    
    SEND_CHAT_ID = db.get_chat_id(user_data[0]["email"].lower())

    update_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    homework_name = (db.get_lesson(lesson_id))["name"]

    tg_user = await message.bot.get_chat(int(tg_id))

    msg_1 = await message.bot.send_message(SEND_CHAT_ID, f'⬇️ {tg_user.full_name} @{tg_user.username} ({user_data[0]["email"].lower()}) сделал ДЗ! Урок: {homework_name} (Техническая информация: {tg_id}_{lesson_id}) ⬇️')
    msg_2 = await message.bot.copy_message(SEND_CHAT_ID, int(tg_id), msg_id)

    db.add_homework(
        f"{tg_user.full_name} @{tg_user.username} {tg_user.id}",
        lesson_id,
        "⏳",
        "",
        update_time,
...
@start_router.message(F.text.startswith('/send_msg_2'))
async def command_start_handler(message: Message) -> None:
    if message.from_user.id != 935173049:
        return
    
    _, tg_id, msg_id, lesson_id = message.text.split()

    user_data = db.get_user(tg_id)
    users_flow = db.get_flow_by_email(user_data[0]["email"].lower())
    
    SEND_CHAT_ID = db.get_chat_id(user_data[0]["email"].lower())
    homework_name = (db.get_lesson(lesson_id, users_flow))["name"]
    ...
    await message.answer(f"{msg_1.message_id} {msg_2.message_id}")

@start_router.message(F.text == 'main')
async def command_start_handler(message: Message) -> None:
    await message.answer(f'config.SHEETS_DATA: {config.SHEETS_DATA}')

@start_router.callback_query(F.data == 'main')
async def command_start_handler(call: CallbackQuery, state: FSMContext) -> None:
    await state.clear()
    await edit_message(call.message, 'Добро пожаловать в бот проверки домашних заданий на курсе "Заработок на Reels"', reply_markup=keyboard.main_keyboard())

@start_router.callback_query(F.data == 'get_support')
async def command_start_handler(call: CallbackQuery) -> None:
    await edit_message(call.message, 'Если у тебя возникли вопросы, то напиши нам обязательно в поддержку, и мы тебе поможем @stepbybit', reply_markup=keyboard.support_keyboard())

"""@start_router.callback_query(F.data == 'get_psychologist')
async def command_start_handler(call: CallbackQuery) -> None:
    await edit_message(call.message, '''На обучении есть психолог, который разберет каждый твой запрос, если будешь сталкиваться со сложностями во время обучения. 

Ты всегда можешь к нему обратиться, заполнив форму: https://forms.gle/s8jf3RuKRxxjZWBV9

Разбор твоей ситуации от психолога придет в телеграм от службы заботы в течение 2 дня после обращения.''', reply_markup=keyboard.psychologist_keyboard())"""

@start_router.callback_query(F.data.startswith('get_psychologist'))
async def command_start_handler(call: CallbackQuery, state: FSMContext) -> None:
    msg_text = '''На обучении есть психолог, который разберет каждый твой запрос, если будешь сталкиваться со сложностями во время обучения. 

Отправь сообщение, чтобы психолог связался с тобой'''

    if len(call.data.split(':')) == 1:
        msg = await edit_message(call.message, msg_text, reply_markup=keyboard.psychologist_keyboard())
    else:
        try:
        ...
        msg = await call.message.answer(msg_text, reply_markup=keyboard.psychologist_keyboard())
    
    await state.set_state(SendPsycho.psycho)
    await state.update_data(bot_message_id=msg.message_id)
    
@start_router.message(StateFilter(SendPsycho.psycho))
async def command_start_handler(message: Message, state: FSMContext) -> None:
    state_data = await state.get_data()

    try:
        await message.bot.edit_message_reply_markup(chat_id=message.chat.id, message_id=state_data['bot_message_id'], reply_markup=None)
    except:
        pass

    #await state.clear()

    msg = await message.answer('Сообщение отправлено, психолог свяжется с тобой в течение 2-ух дней', reply_markup=keyboard.psychologist_keyboard())

    await state.update_data(bot_message_id=msg.message_id)

    user_data = db.get_user(message.from_user.id)

    msg_1 = await message.bot.send_message(config.PSYHOLOGIST_CHAT_ID, f'⬇️ {message.from_user.full_name} @{message.from_user.username} ({user_data[0]["email"].lower()}) отправил сообщение (Техническая информация: {message.from_user.id}) ⬇️')
    await message.bot.copy_message(config.PSYHOLOGIST_CHAT_ID, message.chat.id, message.message_id)

    db.insert_psychologist_question(f"{message.from_user.full_name} @{message.from_user.username} ({message.from_user.id})", user_data[0]["email"].lower(), message.text, f"https://t.me/c/{-(config.PSYHOLOGIST_CHAT_ID+1000000000000)}/{msg_1.message_id}", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
async def command_start_handler(call: CallbackQuery) -> None:
    ...
    if homework_type == 'checking':
        ...
        for i in required_homework: # TODO Сделать в 2 строчки уроки (которые через _ указываются в гугл таблице)
...
async def homework_is_done(call: CallbackQuery) -> None:
    #await call.answer('Вы уже выполнили данное ДЗ', show_alert=True)

    lesson_id = call.data.split(':')[1]
    homework_data = db.get_homework_by_lesson_id(call.from_user.id, lesson_id)
    send_message_id = homework_data[0]["send_message_id"]
    ...
    if not last_solution_text:
        msg_1 = await call.bot.copy_message(chat_id=call.from_user.id, from_chat_id=call.message.chat.id, message_id=send_message_id)
    else:
...
async def command_start_handler(call: CallbackQuery) -> None:
    ...
    if len(text_data) == 1 or homework_text_data[0]['text'] is None:
        ...
        await call.bot.copy_message(chat_id=call.from_user.id, from_chat_id=call.message.chat.id, message_id=homework_data[0]["send_message_id"], reply_markup=keyboard.back_from_promotion_strategy_keyboard())
        return
...
async def command_start_handler(message: Message, state: FSMContext) -> None:
    ...
    if len(homework_data) != 0 and homework_data[0]['status'] == '✅':
        try:
            ...
            pass
        """msg_1 = await message.bot.send_message(SEND_CHAT_ID, f'⬇️ {message.from_user.full_name} @{message.from_user.username} ({user_data[0]["email"].lower()}) <b>отправил комментарий к готовому ДЗ!</b> Урок: {homework_name} (Техническая информация: {message.from_user.id}_{state_data["lesson_id"]}_1) ⬇️', parse_mode='HTML')
        msg_2 = await message.bot.copy_message(SEND_CHAT_ID, message.chat.id, message.message_id)
        await message.answer('Твое сообщение отправлено трекеру ✅', reply_markup=keyboard.back_from_lesson_keyboard(state_data['module_id']))
        await state.clear() TODO"""
        return
    ...
    """
    await state.update_data(homework_text=('' if "homework_text" not in state_data else state_data["homework_text"] + "\n") + message.text)
    update_unix_time = 0 if len(homework_data) == 0 else datetime.datetime.strptime(homework_data[0]["check_time"], "%Y-%m-%d %H:%M:%S").timestamp()
    print(update_unix_time, "homework_text" in state_data)

    if time.time() - update_unix_time < 3 and "homework_text" in state_data: # На случай, если сообщение от пользователя сплитанулось
        document = Document()
        document.add_paragraph(homework_data[0]["comment"])
        file_name = f'cache/ДЗ от {message.from_user.id if message.from_user.username is None else "@" + message.from_user.username}.docx'
        document.save(file_name)
        msg_2 = await message.bot.send_document(SEND_CHAT_ID, FSInputFile(file_name))
        db.change_homework_send_message_id(homework_data[0]["homework_id"], msg_2.message_id)
        return"""
...
async def send_congratulation_message(message: Message, lesson_id, user_id):
...
async def command_start_handler(message: Message) -> None:
    ...
    if message.chat.id == config.PSYHOLOGIST_CHAT_ID and message.reply_to_message is not None and message.reply_to_message.text is not None and message.reply_to_message.text.__contains__('(Техническая информация:'):
        ...
        """try:
            await message.bot.send_message(user_id, message.text, reply_markup=keyboard.psychologist_keyboard_2())
        except:
            pass"""
    ...
    if message.reply_to_message.text.__contains__('(Техническая информация:'):
        ...
        #homework_data = db.get_homework_by_message_ids(message_id, message.chat.id)
        return
    ...
    """if len(technical_info) == 3: TODO
        await message.bot.send_message(
            int(homework_data[0]['user_data'].split()[-1]),
            "Твой персональный трекер проверил твое задание🔥 Нажми на кнопку, чтобы посмотреть обратную связь👇🏻",
            reply_markup=keyboard.open_lesson_keyboard(homework_data[0]['lesson_id'])
        )
        return"""
